...
CONF_FILTER = "filter"
CONF_MAX_AGE_SECONDS = "max_age_seconds"
CONF_NAME = "name"
CONF_SORT = "sort"
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        vol.Optional(CONF_MAX_AGE_SECONDS, default=0): int
    }
)


def get_files_list(folder_path, filter_term, sort, recursive, max_age_seconds):
    """Return the list of files, applying filter."""
    query = folder_path + filter_term
    """files_list = glob.glob(query)"""
    files_list = glob.glob(query, recursive=recursive)

    if max_age_seconds > 0:
        cutoff = datetime.now() - timedelta(seconds=max_age_seconds)
        files_list = filter(lambda f: datetime.utcfromtimestamp(os.path.getmtime(f)) > cutoff, files_list)

    if sort == "name":
        files_list = sorted(files_list)
    elif sort == "size":
        files_list = sorted(files_list, key=os.path.getsize)
    else:
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    if not hass.config.is_allowed_path(path):
        ...
        folder = FilesSensor(
            ...
            config.get(CONF_MAX_AGE_SECONDS)
        )
...
class FilesSensor(Entity):
    ...
    def __init__(self, folder_path, name, filter_term, sort, recursive, max_age_seconds):
